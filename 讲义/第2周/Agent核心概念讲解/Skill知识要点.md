# Agent Skills 知识要点（万字详解 Agent Skills）

> 原文来源：JavaGuide（https://javaguide.cn/ai/agent/skills.html）
> 本文内容为原网页正文的完整搬运，未做改写。
...
![skill-prompt-function-calling-mcp-comparison.webp](images/skill-prompt-function-calling-mcp-comparison.webp)

放在一个真实链路里，大概是这样：

![skill-agent-execution-link.webp](images/skill-agent-execution-link.webp)

- 用户提出任务（Prompt）
- 宿主把可用 Skills 的简短描述放进上下文（Skill 元数据）
- 模型判断当前任务命中了某个 Skill（Skill 路由）
- 宿主再把完整 `SKILL.md` 加载进来（延迟加载）
- 模型按照 Skill 里的流程去调工具、读资料、写结果（执行）
...
![why-does-the-following-content-fail.png](images/why-does-the-following-content-fail.png)
...
![keep-skill-md-content-under-500-lines-for-best-performance.png](images/keep-skill-md-content-under-500-lines-for-best-performance.png)
...
![skillssh.png](images/skillssh.png)

![superpowers-skills.png](images/superpowers-skills.png)
...
![agent-skills-progressive-disclosure.webp](images/agent-skills-progressive-disclosure.webp)
...
![skills-progressive-disclosure.svg](images/skills-progressive-disclosure.svg)

就像查书一样。你不会先把整本书背下来，而是先看目录，确定章节，再翻到具体那一页。
一般可以分成三层：

![skills-progressive-disclosure-three-layer-model.png](images/skills-progressive-disclosure-three-layer-model.png)
...
![agent-skills-workflow-design.webp](images/agent-skills-workflow-design.webp)
...
![agent-skills-routing-flow.webp](images/agent-skills-routing-flow.webp)
...
![skills-router.svg](images/skills-router.svg)
