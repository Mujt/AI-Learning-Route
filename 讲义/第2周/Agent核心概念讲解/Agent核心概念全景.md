...
> 原文来源：JavaGuide（https://javaguide.cn/ai/agent/agent-basis.html）
...
![agent-core-arch.png](images/agent-core-arch.png)
...
![agent-loop-flow.png](images/agent-loop-flow.png)
...
![mcp-simple-diagram.png](images/mcp-simple-diagram.png)
...
![context-engineering-vs-context-engineering-dimension-comparison.png](images/context-engineering-vs-context-engineering-dimension-comparison.png)
...
![ReAct-LLM.png](images/ReAct-LLM.png)
...
![agent-react-flow.png](images/agent-react-flow.png)
...
![agent-multi-agent-arch.png](images/agent-multi-agent-arch.png)

当任务确实能按专业角色拆分时，Multi-Agent 可以并行执行，且单个子任务失败未必阻断整体。代价是 Agent 间的通信、协调和调试成本都会上升，Token 消耗也随之增加。
落地时还要处理任务契约、共享状态、并行写冲突、Worker 接管和检查点恢复。详细设计可以看 [《多 Agent 协作系统设计：任务拆分、状态共享、冲突处理与失败恢复》](/ai/agent/multi-agent.html)。
...
![agent-a2a.png](images/agent-a2a.png)

### Agentic Workflows

Agentic Workflows 是吴恩达（Andrew Ng）重点倡导的概念，强调用工程编排把推理、工具、记忆、反思和多实体协作接成可执行流程，而不只等待底层模型能力变化。

![agent-agentic-workflows.png](images/agent-agentic-workflows.png)
