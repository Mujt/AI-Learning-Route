...
> **原文来源**：[JavaGuide - RAG 向量索引算法和向量数据库](https://javaguide.cn/ai/rag/rag-vector-store.html)（作者：Guide）
> 本文为网页原文的完整抓取转换：正文文字、表格、代码块均按原文保留，图片已下载到本目录 `images/` 下，Mermaid 图表以源码形式嵌入。
...
基础概念可以看 [RAG 基础篇](/ai/rag/rag-basis.html)。本文重点放在后半段：这些向量怎么高效存储、索引和检索。
